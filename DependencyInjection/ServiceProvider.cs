...
public sealed class ServiceProvider : IServiceProvider, IScopeCapableServiceProvider
{
    ...
    private static object InstantiateService(IServiceProvider services, Type implementationType, Guid scopeId)
    {
        ...
        foreach (var parameterInfo in dependencies)
        {
            ...
            if (paramType.IsInstanceOfType(services))
            {
                param = services;
            }
            else if (paramType.IsAssignableTo(typeof(Guid)) && Attribute.IsDefined(parameterInfo, typeof(ScopeIdAttribute)))
            {
                param = scopeId;
            }
            else
